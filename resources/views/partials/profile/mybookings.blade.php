...
<div class="bg-white rounded-3xl shadow-xl p-8" data-aos="fade-up" data-aos-delay="100">
    <div class="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-6">
        <div>
            <h2 class="text-2xl font-bold text-gray-800 font-poppins">Booking</h2>
        </div>
    </div>
...
<div id="bookingsContainer" class="space-y-6">
    @forelse($bookings as $index => $booking)
        <div class="booking-card bg-white rounded-2xl border border-gray-200 p-6 hover:shadow-lg transition-all duration-300"
            ...
            <div class="flex items-start justify-between mb-4 pb-4 border-b border-gray-100">
                <div class="flex items-center gap-3">
                    ...
                    <div class="w-12 h-12 bg-gray-100 rounded-xl flex items-center justify-center flex-shrink-0">
                        @if(str_contains(strtolower($booking->area), 'glamping') || isset($booking->deck))
                    ...
                    <div>
                        <h3 class="font-bold text-gray-900 font-poppins">{{ $booking->area_type ?? 'Glamping' }}</h3>
                        <p class="text-sm text-gray-500 font-poppins">{{ \Carbon\Carbon::parse($booking->check_in)->format('d F Y') }}</p>
                    </div>
                </div>
                
                {{-- Status Badge --}}
                <span class="px-3 py-1 rounded-full text-xs font-semibold font-poppins
                    {{ $booking->status === 'confirmed' ? 'bg-[#017249] text-white' : '' }}
                    {{ $booking->status === 'pending' ? 'bg-yellow-500 text-white' : '' }}
                    {{ $booking->status === 'failed' ? 'bg-red-500 text-white' : '' }}">
                    {{ $booking->status === 'confirmed' ? 'Berhasil' : ($booking->status === 'pending' ? 'Proses' : 'Gagal') }}
                </span>
            </div>
            ...
            <div class="grid md:grid-cols-2 gap-6 mb-4">
                <div class="space-y-2 text-sm font-poppins">
                    ...
                    <div class="flex items-center">
                        ...
                        <span class="text-gray-900 font-medium">: {{ $booking->guests }} Orang</span>
                    </div>
                </div>
                
                <div class="text-right md:text-right">
                    <p class="text-sm text-gray-600 font-poppins mb-1">Total Harga</p>
                    <p class="text-2xl font-bold text-gray-900 font-poppins">Rp. {{ number_format($booking->total, 0, ',', '.') }}</p>
                </div>
            </div>
            ...
            <div class="flex gap-3 pt-4 border-t border-gray-100">
                ...
                <button class="flex-1 px-4 py-2.5 bg-[#017249] text-white rounded-lg hover:bg-[#015a3a] font-semibold transition-all duration-300 font-poppins text-sm">
                    {{ $booking->status === 'confirmed' ? 'Pesan Lagi' : 'Hubungi Admin' }}
                </button>
            </div>
        </div>
    @empty
        <div class="bg-white rounded-3xl shadow-xl p-12 text-center" data-aos="fade-up">
            ...
            <h3 class="text-xl font-bold text-gray-800 mb-2 font-poppins">Belum Ada Booking</h3>
            <p class="text-gray-500 mb-6 font-poppins">Anda belum memiliki booking aktif</p>
            <a href="{{ route('reservation') }}" class="inline-block px-8 py-3 bg-[#017249] text-white rounded-lg hover:bg-[#015a3a] font-semibold transition-all duration-300 font-poppins text-sm">
                Buat Reservasi
            </a>
        </div>
    @endforelse
